...
class WGAN:
    ...
    def build_generator(self):
        ...
        model = Sequential()
        model.add(Conv1D(filters=32, kernel_size=16, input_shape=(self.latent_dim, 1)))
    ...
    def build_critic(self):
        ...
        model = Sequential()
        model.add(Conv1D(filters=32, kernel_size=16, input_shape=(self.matrix_shape)))
        model.add(BatchNormalization())
        model.add(Activation(activation='relu'))
    ...
    def train(self, epochs, batch_size=128):
        ...
        for epoch in range(epochs):
            ...
            if np.isnan(1 - d_loss[0]) or np.isnan(1 - g_loss[0]):
                break

            if epoch >= self.epoch_at_sarting_generate_data and epoch % self.skip_val_data_generate == 0:
                self.get_cosin_similarity(batch_size=1, no_of_images=20)
    ...
    def get_cosin_similarity(self, batch_size, no_of_images):

        temp_gen_arr = []
        for g in range(no_of_images):
            batch_arr = []
            noise = np.random.normal(size=(batch_size, self.latent_dim, 1))
    ...
    def read_file(self, file_name, category, margin, option, trace):
        data = trace
        if option == 0:
    ...
    def assign_parameters(self, option, margin):

        self.margin = margin

        if option == 0:
            self.epoch_at_sarting_generate_data = 50
...
def main(file_name, category, margin, option, epochs, trace):
    ...
    try:
        ...
        with tf.device('/device:GPU:1'):
            aae = WGAN()
            aae.read_file(file_name, category, margin, option, trace)
            aae.assign_parameters(option, margin)
